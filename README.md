...
```shell
  docker buildx build -t moby-metrics/topmast:latest . --load
```

To install the extension, use `make install-extension` **or**:

```shell
  docker extension install moby-metrics/topmast:latest
```
...
```shell
  docker extension dev ui-source moby-metrics/topmast:latest http://localhost:3000
```

In order to open the Chrome Dev Tools for your extension when you click on the extension tab, run:

```shell
  docker extension dev debug moby-metrics/topmast:latest
```

Each subsequent click on the extension tab will also open Chrome Dev Tools. To stop this behaviour, run:

```shell
  docker extension dev reset moby-metrics/topmast:latest
```
...
```shell
docker extension update moby-metrics/topmast:latest
```
...
```shell
docker extension rm moby-metrics/topmast:latest
```
